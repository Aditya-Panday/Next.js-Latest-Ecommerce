...
const Maincategories = ({ main, itemsPerPage, handleItemsPerPageChange }) => {
  return (
    <>
      ...
      <div className="container mx-auto ">
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          <table className="min-w-full table-auto">
            ...
            <tbody>
              {main.map((main) => (
                <tr key={main.id} className="border-t border-gray-200">
                  <td className="py-3 px-4 text-sm font-medium">{main.id}</td>
                  <td className="py-3 px-4 text-sm">{main.name}</td>
                  <td className="py-3 px-4 text-sm">{main.category}</td>
